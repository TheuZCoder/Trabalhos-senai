...
function TodoList() {
    ...
    const deleteTask = (index) => {
        ...
        // Define uma função 'deleteTask' que remove uma tarefa com baseem seu índice na lista.
...
clicado. */}
            <ul>
                {tasks.map((task, index) => (
                    // Renderiza uma lista não ordenada (ul) para exibiras tarefas.

                    // Usa 'map' para criar um item de lista (li) para cada tarefa no array 'tasks'.

                    // 'key={index}' é usado para ajudar o React aacompanhar os elementos da lista.
                    <li key={index}>
                        {task}
